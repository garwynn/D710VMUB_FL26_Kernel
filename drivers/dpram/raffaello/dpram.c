...
*/
static void fmt_res_ack_tasklet_handler(unsigned long data);
static void raw_res_ack_tasklet_handler(unsigned long data);
...
static DECLARE_TASKLET(fmt_res_ack_tasklet, fmt_res_ack_tasklet_handler, (unsigned long)&dpram_table[FORMATTED_INDEX]);
static DECLARE_TASKLET(raw_res_ack_tasklet, raw_res_ack_tasklet_handler, 0);
...
static void raw_res_ack_tasklet_handler(unsigned long data)
{
    ...
    pr_err("[DPRAM] <%s> Called!!\n", __func__);

    for (slot = 0; slot < MAX_PDP_CONTEXT; slot++) {
	    dev = pdp_table[slot];
	if (dev != NULL  && dev->vs_dev.tty != NULL && dev->vs_dev.refcount > 0) {
		struct tty_struct *tty = dev->vs_dev.tty;
	
		//pr_err("[DPRAM] <%s> tty_driver name = %s [%u] \n", __func__, tty->driver->name, dev->id);
